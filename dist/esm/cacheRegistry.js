 ...
 */
export const getOrSetRuleTree = (schema, compileFn) => {
    if (!schema) {
 ...
 */
export const makeOverrideKey = (overrides = {}) => {
    const overrideData = {
 ...
 */
export const getJsonSchemaCache = (schema) => {
    if (!schema) {
 ...
 */
export const getAjvValidatorCache = (schema) => {
    if (!schema) {
        // In "no-schema" mode, fallback to global Map keyed by overrideKey.
