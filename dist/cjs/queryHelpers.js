...
exports.parsePagination = exports.parseSort = exports.buildProjection = exports.buildFilter = void 0;
/**
...
const buildFilter = (params = {}) => {
    ...
    if (params.search?.q && params.search.fields?.length) {
        const regex = new RegExp(params.search.q, 
 ...
 */
const buildProjection = ({ include = [], exclude = [], } = {}) => {
    const proj = {};
    // Include mode: mark selected fields with 1
    if (include.length)
        include.forEach((f) => (proj[f] = 1));
    // Exclude mode: mark selected fields with 0
    if (exclude.length)
        exclude.forEach((f) => (proj[f] = 0));
    return proj;
};
 ...
 */
const parsePagination = ({ page = 1, pageSize = 20, } = {}) => {
