...
export type ExistsOperator = { $exists: boolean };
export type RangeOperator<T> = {
  $gte?: T extends number | Date ? T : never;
  $lte?: T extends number | Date ? T : never;
};
export type InOperator<T> = { $in: readonly T[] };
export type RegexOperator = { $regex: string | RegExp };
...
export type Condition<V> =
  | V                               // simple equality
  | (V extends string ? RegexOperator : never)
  | (V extends Primitive ? ExistsOperator : never)
  | (V extends number | Date ? RangeOperator<V> : never)
  | InOperator<V>;
