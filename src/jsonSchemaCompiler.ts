import type { Rule, RuleTree, Overrides } from './types.js';
 ...
 */
const mergeRules = (base: Rule, overlay: Partial<Rule>): Rule => ({
  ...base,
  ...overlay,
});
...
const flatten = (
  ...
  for (const [k, child] of Object.entries(tree.children || {})) {
    ...
    if (child.type === 'object' && child.children) {
      flatten(
        { type: 'object', children: child.children },
        prefix ? `${prefix}.${k}` : k,
        out
      );
    } else {
...
export const applyOverrides = (
  ...
  const effective: Record<string, Rule> = {};
  for (const [path, rule] of Object.entries(baseMap)) {
    ...
    const r: Rule = { ...rule };
    if (optional.has(path)) r.required = false;
    if (required.has(path)) r.required = true;
  ...
  const root: RuleTree = { type: 'object', children: {} };
 ...
 */
export const toJsonSchema = (tree: RuleTree, allowUnknown = false): any => {
  /**
   * Recursively convert a Rule node to its JSON Schema equivalent.
   */
  const nodeToSchema = (node: Rule): any => {
    ...
    if (node.type === 'array') {
      ...
      if (node.minItems != null) schema.minItems = node.minItems;
      if (node.maxItems != null) schema.maxItems = node.maxItems;
      return schema;
    }
    ...
    if (node.type === 'string') {
      ...
      if (node.minLength != null) schema.minLength = node.minLength;
      if (node.maxLength != null) schema.maxLength = node.maxLength;
      if (node.pattern)
        schema.pattern =
          typeof node.pattern === 'string'
            ? node.pattern
            : (node.pattern as RegExp).source;
      if (node.enum) schema.enum = node.enum.slice();
      return schema;
    }
    ...
    if (node.type === 'number') {
      ...
      if (node.min != null) schema.minimum = node.min;
      if (node.max != null) schema.maximum = node.max;
      return schema;
    }
    ...
    if (node.type === 'date') return { type: 'string', format: 'date-time' };
