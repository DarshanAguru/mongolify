import type { Rule, RuleTree } from './types.js';
 ...
 */
const mapInstanceToType = (instance: string): Rule['type'] => {
  switch (instance) {
    case 'String':
      return 'string';
    case 'Number':
 ...
 */
const setNested = (root: RuleTree, path: string, value: Rule) => {
  ...
  for (let i = 0; i < parts.length - 1; i++) {
    ...
    cur.children[seg] ||= { type: 'object', children: {} } as Rule;
 ...
 */
export const compileRulesFromMongooseSchema = (schema: any): RuleTree => {
  // Initialize the root object node
  const ruleTree: RuleTree = { type: 'object', children: {} };
  ...
  schema.eachPath((path: string, schematype: any) => {
    ...
    if (type === 'string') {
      ...
      if (opts.minlength != null) rules.minLength = opts.minlength;
      if (opts.maxlength != null) rules.maxLength = opts.maxlength;
    }

    // NUMBER constraints
    if (type === 'number') {
      if (opts.min != null) rules.min = opts.min;
      if (opts.max != null) rules.max = opts.max;
    }

    // DATE constraints (stored as timestamps for easy comparisons in runtime validators)
    if (type === 'date') {
      if (opts.min != null) rules.min = new Date(opts.min).getTime();
      if (opts.max != null) rules.max = new Date(opts.max).getTime();
    }
    ...
    if (type === 'array') {
      ...
      if (caster) {
        ...
        rules.items = { type: itemType } as Rule;

        // Item-level options for strings/numbers (from caster.options)
        const cOpts = caster.options || {};
        if (itemType === 'string') {
          if (cOpts.enum) (rules.items as Rule).enum = cOpts.enum.slice();
          if (cOpts.match)
            (rules.items as Rule).pattern =
              cOpts.match instanceof RegExp
                ? cOpts.match
                : new RegExp(cOpts.match);
          if (cOpts.minlength != null)
            (rules.items as Rule).minLength = cOpts.minlength;
          if (cOpts.maxlength != null)
            (rules.items as Rule).maxLength = cOpts.maxlength;
        }
        if (itemType === 'number') {
          if (cOpts.min != null) (rules.items as Rule).min = cOpts.min;
          if (cOpts.max != null) (rules.items as Rule).max = cOpts.max;
        }

        // Array of subdocuments: caster.schema exists → items become object with children
        if (caster.schema) {
          (rules.items as Rule).type = 'object';
          (rules.items as Rule).children = compileRulesFromMongooseSchema(
            caster.schema
          ).children;
        }
      }
      ...
      if (opts.minItems != null) rules.minItems = opts.minItems;
      if (opts.maxItems != null) rules.maxItems = opts.maxItems;
    }

    // EMBEDDED SUBDOCUMENT: schematype.schema exists → nested object with children
    if (schematype.schema) {
      rules.type = 'object';
      rules.children = compileRulesFromMongooseSchema(
        schematype.schema
      ).children;
    }
